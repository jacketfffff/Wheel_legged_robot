...
NavCore::NavCore(std::string base_foot_print, std::string map_frame):BASE_FOOT_PRINT_(std::move(base_foot_print)),MAP_FRAME_(std::move(map_frame))
{
    ...
    if(!(isMaveBaseClientConnected_ = moveBaseClient -> isServerConnected()))
        ROS_ERROR_STREAM("MVOEBASE ACTION SERVER FAILED");
    else
        ROS_INFO_STREAM("MOVEBASE ACTION SERVER CONNECTED");
...
const geometry_msgs::Pose2D & NavCore::getCurrentPose(const std::string &target_frame, const std::string &source_frame)
{
    ...
    try
    {
        transformStamped = tfBuffer_.lookupTransform(target_frame, source_frame, ros::Time(0), ros::Duration(3.0));
    }
